 ...
 * @brief CPU parallel implementations of the functions used in material segmentation.
 * @version 0.1
 * @date 2024-09-17
 *
...
namespace cpu_par {
    ...
    inline double resample2x2x2(const field_type *voxels,
        ...
        for(int i = 0; i < 3; i++) {
            double Iminus, Iplus;
            Xfrac[0][i] = 1-std::modf(X[i]-0.5, &Iminus); // 1-{X[i]-1/2}, floor(X[i]-1/2)
            Xfrac[1][i] =   std::modf(X[i]+0.5, &Iplus);  // {X[i]+1/2}, floor(X[i]+1/2)
        ...
        for (int ijk = 0; ijk <= 7; ijk++) {
            ...
            for (int axis = 0; axis < 3; axis++) { // x +- 1/2
                int pm = (ijk>>axis) & 1;
        ...
        for (uint64_t z = sz; z < Nz; z++) {
            for (uint64_t y = sy; y < Ny; y++) {
                for (uint64_t x = sx; x < Nx; x++) {
                    ...
                    int64_t voxel_index = (int64_t) std::floor(static_cast<double>(Nvoxel_bins-1) * ((voxel - v_min) / double(v_max - v_min)));
                    ...
                    int64_t field_index = (int64_t) std::floor(static_cast<double>(Nfield_bins-1) * (double(field_value - f_min) / double(f_max - f_min)) );
