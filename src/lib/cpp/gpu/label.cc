        ...
        for (uint64_t z = sz; z < Nz; z++) {
            for (uint64_t y = sy; y < Ny; y++) {
                for (uint64_t x = sx; x < Nx; x++) {
                    ...
                    int64_t voxel_index = (int64_t) floor(static_cast<float>(Nvoxel_bins-1) * ((voxel - v_min)/float(v_max - v_min)) );
                    ...
                    int64_t field_index = (int64_t) floor(static_cast<float>(Nfield_bins-1) * (float(field_value - f_min)/float(f_max - f_min)) );
